...
static BOOL GarbageCollector(void)
{
  ...
  for (image=(struct ImageData *)ImageList.lh_Head ;
	...
	{
	  /* leider nichts m�glich, da es benutzt wird ! */
	}
...
BOOL FloydDither(LONG x,LONG y,UBYTE r,UBYTE g,UBYTE b)
{
  ...
  red_dist=green_dist=blue_dist=0; /* schon verteilten Fehler r�cksetzen */
  ...
  {
    setx=0; sety=y+1;	  /* wenn rechts am Rand, dann auf n�chste Zeile links */
  }
  if (sety <= MaxImageY)
  {
