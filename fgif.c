...
static BOOL WriteCode(short code,int codelength)
{
  ...
  while (codeleft>0)
  {
    ...
    /* Bits ausschneiden, so da� sich die Bits nachher ganz rechts befinden */
    ...
    /* Bits zu dem Byte links hinzuf�gen */
...
static void InitCodeTable(int codesize)
{
 ...
 // codetable[i].next wird bei den ersten 1<<codesize Eintr�gen nie benutzt */
  }
}
...
/*
  Algorithmus:
    Die ersten 1<<codesize Eintr�ge werden ganz normal als Tabelle benutzt.
    Alle weiteren Tabelleneintr�ge, die x als Prefix haben, sind durch
    x->childs erreichbar, verkettet durch ->next.

    Die beiden Eintr�ge bei den Spezial-Codes bleiben unbenutzt.
*/
...
BOOL SaveGIF(void)
{
  ...
  /* Graphic Control Extension schreiben, wenn Transparency ben�tigt wird */
