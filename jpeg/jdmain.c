...
#endif
#ifdef PPM_SUPPORTED
  fprintf(stderr, "  -pnm           Select PBMPLUS (PPM/PGM) output format\n");
#endif
...
#endif
  fprintf(stderr, "  -retina        Display picture on retina (default)\n");
 ...
 */
{
  ...
  for (argn = 1; argn < argc; argn++) {
    ...
    } else if (keymatch(arg, "retina", 1)) {
      /* retina display */
      requested_fmt = FMT_RETINA;
